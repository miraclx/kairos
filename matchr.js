...
let [lastWrite, lastTicked, spinCursor] = [Date.now(), 0, 0];
let found, now;
for (let potential_phrase of permute(valids)) {
  current += 1;
  let phrase = potential_phrase.join(' ');
  let isValid = bip39.validateMnemonic(phrase, bip39English);
  let keyPair =
    isValid &&
    (state[phrase] ||
